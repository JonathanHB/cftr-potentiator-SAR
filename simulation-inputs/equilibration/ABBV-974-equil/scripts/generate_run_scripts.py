...
input_folder = "input"
output_folder = "/wynton/home/grabe/csheen/cftr-project/cftr-glpg-unbinding/run05/run_scripts/" # the folder to which to output the generated scripts, not the folder for md output
run_folder = '/wynton/home/grabe/csheen/cftr-project/cftr-glpg-unbinding/run05/output'
...
# 	f.write(sim_command_minim.format(gmx = gmx, upperdir = cwd+"/../run05", mdp=current_mdp, current=current_sim, previous=previous_sim, inp=initial_structure, input_folder=input_folder))
# with open(output_folder+runs_file, 'a') as f:
# 	f.write(file_prefix+current_sim+file_suffix+'\n')

previous_sim = 'prod009'
...
# for i in range(14,len(mdp_files)):
# 	current_sim="eq"+str(i)
# 	current_mdp = mdp_files[i] #file_prefix+current_sim+mdp_suffix 
# 	with open(output_folder+file_prefix+current_sim+file_suffix, 'w') as f:
# 		f.write(sim_command_equil.format(gmx = gmx, upperdir = cwd+"/../run05", mdp=current_mdp, current=current_sim, previous=previous_sim, inp=initial_structure, input_folder=input_folder))
# 	with open(output_folder+runs_file, 'a') as f:
# 		f.write(file_prefix+current_sim+file_suffix+'\n')
...
for i in range(10,200):
	...
	with open(output_folder+file_prefix+current_sim+file_suffix, 'w') as f:
		f.write(sim_command_prod.format(gmx = gmx, upperdir = cwd+"/../run05", mdp=current_mdp, current=current_sim, previous=previous_sim, input_folder=input_folder))	
	with open(output_folder+runs_file, 'a') as f:
		f.write(file_prefix+current_sim+file_suffix+'\n')
	previous_sim = current_sim
